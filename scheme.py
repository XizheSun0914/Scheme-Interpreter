...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Incorrect number of values passed in.")
        while formals != nil:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** REPLACE THIS LINE ***"
        args = operands.map(lambda op: scheme_eval(op, env))
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        formals_pairs = Pair(bindings.first.first, formals_pairs)
        vals_pairs = Pair(scheme_eval(bindings.first.second.first, env), vals_pairs)
        bindings = bindings.second
    check_formals(formals_pairs)
    return env.make_child_frame(formals_pairs, vals_pairs)
    # END PROBLEM 15
